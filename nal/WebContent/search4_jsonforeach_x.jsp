...
<!-- 탭추가, 해당처리방식 보류, 20211124 KHJ-->
	...
	<!-- 학술지종류 추가, 20211224 KHJ -->
        ...
        <c:choose>
			...
			<c:when test="${(count == 1) && (jsoninfo == null)}">
				<tr>
					<td>책이 없습니다.</td>					
				</tr>
			</c:when>
			<c:when test="${(count == 1) && (jsoninfo != null)}">
				<ul class="list">
					<c:forEach var="list" items="${jsoninfo}">
						<li><a href='javascript:searchInnerDetail('MONO1201539978', 'Y')' title='상세 바로가기'>						
							<c:out value="${list.book_title}"/> / 					
							<c:out value="${list.author}"/>
							<img src='./images/ko/ico/star_30.png' alt='인기도' /></a>
						<ul>
							<li><c:out value="${list.publishing}"/></li>
							<li><c:out value="${list.book_sorting}"/></li>
							<li><c:out value="${list.room_name}"/></li>
							<li></li>
						</ul>				
					</c:forEach>
				</ul>	
