			...
			$.ajax({
				...
				success:function (data, textStatus){	//전송과 응답이 성공했을 경우 작업 설정
					...
					for(var i in jsoninfo.books) {
						...
						bookinfo += "</ul><a href='#none' onclick='fn_myLibrary('list');' title='내서재담기'><img src='./images/ko/ico/briefIco1.png' alt='인기도' /></a></li>";
					}
					bookinfo += "</ul>" 
			...
			<c:when test="${(count == 0) && (infoArr != null)}">
				...
				<c:forEach var="list" items="${infoArr}">
					<li><a href='javascript:searchInnerDetail('MONO1201539978', 'Y')' title='상세 바로가기'>
						...
						<a href='#none' onclick='fn_myLibrary('list');' title='내서재담기'><img src='./images/ko/ico/briefIco1.png' alt='인기도' /></a>
						</li>			
				</c:forEach>
				</ul>		
			</c:when>		
